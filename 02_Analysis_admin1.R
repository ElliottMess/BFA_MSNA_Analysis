...
freq_admin1 <- cleaned_data_adm1%>%
  ...
  distinct()%>%
  select(admin1,variable, variable_value, numbers)
...
summary_stats_admin_1_final <- bind_rows(summary_stats_admin_1, freq_admin1)%>%
  left_join(which_subsets_adm1, by = c("admin1", "variable"))%>%
...
write_csv(summary_stats_admin_1_final, "outputs/tables/summary_stats_admin_1.csv")
