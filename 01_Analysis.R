...
added_indicators <- analysisplanTemplate%>%
  ...
  separate(dependent.variable, into = c("question", "choices"), sep = "\\.", remove = FALSE)%>%
  mutate(type = case_when(is.na(choices) & dependent.variable.type == "numerical" ~ "decimal",
                          !is.na(choices) ~ paste0("select_multiple liste_", question),
                          TRUE ~ choices
                          ))%>%
...
added_indicators_survey <- added_indicators%>%
  ...
  rename(name = question, label = label_indicator)%>%
...
analysisplan_admin_1_grp_reduced <- analysisplan_admin_1_grp%>%
  filter(
...
final_result_admin_1_grp <- from_analysisplan_map_to_output(data = cleaned_data_adm1, 
                                                            analysisplan = analysisplan_admin_1_grp_reduced, 
...
summary_stats_admin_1_grp_final <- rbind(summary_stats_admin_1_grp, freq_admin1_grp)%>%
  mutate(question_choice = case_when(is.na(variable_value) ~ variable,
                                     TRUE ~ paste0(variable, ".", variable_value)),
         status = case_when(status == "pdi" ~ "PDI",
                            status == "host" ~ "Communauté hôte",
                            TRUE ~ NA_character_))%>%
  left_join(dico, by = "question_choice")%>%
  mutate(numbers = case_when(dependent.variable.type == "categorical" ~ round(numbers*100,0),
                             dependent.variable.type == "numerical"  ~ round(numbers, 1),
                             TRUE ~ numbers))%>%
  select(research.question_label, sub.research.question_label, admin1, status, label_indicator, label_choice, numbers)%>%
  ...
  select(research.question_label, sub.research.question_label, admin1Name, status, label_choice, numbers)%>%
  distinct()%>%
...
summary_stats_admin_2_grp_final_grp <- rbind(summary_stats_admin_2_grp, freq_admin2_grp)%>%
  ...
  mutate(numbers = case_when(dependent.variable.type == "categorical" ~ round(numbers*100,0),
                             dependent.variable.type == "numerical"  ~ round(numbers, 1),
                             TRUE ~ numbers))%>%
  select(research.question_label, sub.research.question_label, admin2, status, label_indicator, label_choice, numbers)%>%
  left_join(bfa_admin2, by = c("admin2" = "admin2name"))%>%
  distinct()%>%
  mutate(label_choice = case_when(is.na(label_choice) ~ label_indicator, 
                                  TRUE ~ paste(label_indicator, label_choice, sep = ": ")))%>%
  select(research.question_label, sub.research.question_label, admin1Name, admin2Name, status, label_choice, numbers)%>%
  distinct()%>%
  group_by(research.question_label, sub.research.question_label, admin1Name, admin2Name, status, label_choice)%>%
  mutate(row = row_number())%>%
  pivot_wider(names_from = c(admin1Name, admin2Name), values_from = numbers)%>%
  ungroup()

names(summary_stats_admin_2_grp_final_grp)[1:3] <- c("Région", "Province", "Groupe de population")
